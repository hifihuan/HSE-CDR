...
import utils.hico_text_label as hico_text_label
...
class HICODet(ImageDataset):
    ...
    def object_class_to_target_class(self):
        if self.args.num_classes == 117:
            object_to_target = self.object_to_verb
        elif self.args.num_classes == 600:
            object_to_target = self.object_to_interaction
        if 'train' in self.root and self.args.zs:
            object_to_target = self.zs_object_to_target
