...
class DistributedLearningEngine(State):
    ...
    def __init__(self,
        ...
        # Enable static graph for DDP when using gradient checkpointing
        # This fixes the "Expected to mark a variable ready only once" error
        # when gradient checkpointing is used with DDP
        try:
            self._state.net._set_static_graph()
        except AttributeError:
            # _set_static_graph() is available in PyTorch 1.11+
            # If not available, skip (older PyTorch versions)
            pass
